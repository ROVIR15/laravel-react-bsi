...
class ProductionStudyInNumbersViewTable extends Migration
{
    ...
    public function up()
    {
        ...
        AS
        select `bsi_db`.`production_study`.`id` AS `id`,`bsi_db`.`work_center`.`id` AS `work_center_id`,`product_information_view`.`id` AS `product_id`,`bsi_db`.`work_center`.`name` AS `work_center_name`,`product_information_view`.`name` AS `product_name` from ((`bsi_db`.`production_study` join `bsi_db`.`work_center` on((`bsi_db`.`work_center`.`id` = `bsi_db`.`production_study`.`work_center_id`))) join `bsi_db`.`product_information_view` on((`product_information_view`.`id` = `bsi_db`.`production_study`.`id`)))
        ");
    }
