...
function FinanceAccountShow() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      try {
        API.updateAFinanceAccount(id, {
          financial_account_type_id: values.type,
  ...
  useEffect(() => {
    try {
      API.getAFinanceAccount(id, function(res){
        if(!res) return undefined;
        if(isUndefined(res.data)) throw new Error('data is empty');
        else {
          setFieldValue('name', res.data.account_name)
          setFieldValue('type', res.data.type.id)
          setFieldValue('number', res.data.account_number)
        }
      })
    } catch (error) {
      alert(error)
...
export default FinanceAccountShow;
