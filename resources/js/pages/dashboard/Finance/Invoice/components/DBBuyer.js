...
function SimpleDialog(props) {
  ...
  return (
    <DialogStyled onClose={handleClose} open={open} fullWidth>
      <DialogTitle>Choose Buyer</DialogTitle>
      <List sx={{ pt: 0 }}>
        <Scrollbar sx={{
        ...
        { 
          ...
          opts.map((item) => (
            ...
            >
              ...
              <Typography component="span" variant="caption">{item.street + ` ${item.city}` + ` ${item.province}` + ` ${item.country}`}</Typography>
              <Typography component="span" variant="caption">{item.postal_code}</Typography>
            </ListItemStyled>
          )))
        }
        </Scrollbar>
