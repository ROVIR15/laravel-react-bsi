        ...
        <FormGroup>
          {!opts
            ...
            : filteredData.map((item) => {
                const _checked = selected.map((e) => e.id).indexOf(item.id) !== -1;
                return (
                  <_ListItemStyled
                    control={<Checkbox checked={_checked} onChange={(event) => handleAddInvoice(event, item)} />}
                    label={<Typography variant="body1">{item.serial_number}</Typography>}
                  />
                );
                // <ListItemStyled
                //   button
                ...
                //   </Typography>
                //   {/* <Typography component="span" variant="caption">{item.billed_to}</Typography> */}
                //   <Typography variant="body2">{`${item.billed_to}`}</Typography>
                // </ListItemStyled>
              })}
        </FormGroup>
      </Scrollbar>
