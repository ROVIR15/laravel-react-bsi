...
function TableD({ list, placeHolder, selected, setSelected, update, close }) {
  ...
  const handleClick = (event, name) => {
    const _data = name.items.map(function (item, index) {
            ...
            <TableBody>
              {filteredData
                ...
                .map((row) => {
                  ...
                  const { id, items } = row;
                  let item_name = `${row?.product_feature?.product?.goods?.name} - ${row?.product_feature?.color}`;
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">{items?.length}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
