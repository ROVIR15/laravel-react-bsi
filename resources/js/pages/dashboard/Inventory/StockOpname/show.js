...
import { adjustment_data_prep } from '../utils'
...
function StockAdjustment() {
  ...
  // get id of url, identify that the url is showing 
  // spesific data based on id
  const { id } = useParams();
  ...
  useEffect(() => {
    API.getAnAdjustment(id, function(res){
      if(!res) return;
      if(isEmpty(res.data)) {
        ...
        setValues({
          ...
          adjustment_date: res.data?.date,
          adjustment_type: res.data?.change_type,
          description: res.data?.description
        });

        let _items = adjustment_data_prep(res.data.items);
