...
export default function BasicModal({ payload, order_id, updateOrderItem, open, options, handleClose, setComponent}) {
  ...
  const handleDoneFill = () => {
    ...
    const _value = value.map(function(x, index){
      return {...x, product_feature_id: x.id, id: payload.length+index, order_id, qty: 0, unit_price: 0}
    })

    API.insertSalesOrderItem(_value, function(res){
