...
export default function BasicModal({ id, open, handleClose, setComponent}) {
  ...
  const isEmpty = () => {
    return value.labor_id === '' && value.process_id === '';
  }
  ...
  const submit = (value) => {
    API.insertProcessStudy({...value, production_study_id: id}, function(res) {
      if(!res.success) {
        alert('Failed');
      } else {
        alert('Success');
