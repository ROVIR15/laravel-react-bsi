    ...
    let result = data?.reduce(function (initial, next) {
      ...
      let targeted = initial?.map((item, index) => {
        if (item.name.toLowerCase() === next?.status[0]?.status_type?.toLowerCase())
          return {
            ...item,
            amount: item.amount + parseFloat(next?.sum[0]?.total_money),
  ...
  let total = data?.reduce(
    function (initial, next) {
      return {
        ...
        amount: initial.amount + parseFloat(next?.sum[0]?.total_money),
      };
    },
          ...
          <StyledComponent>
            ...
            <StyledComponentInformation>
              ...
              <StyledTypography variant="h6">
                ...
                <StyledSpanTypography>purchase order</StyledSpanTypography>
              </StyledTypography>
