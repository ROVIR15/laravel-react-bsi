...
export default function SummaryPaper({ data, type }) {
  ...
  useEffect(() => {
    ...
    let result = data?.reduce(function (initial, next) {
      ...
      let targeted = initial?.map((item, index) => {
        let last = index === initial.length-1; 
        if(next?.status?.length===0 && last){
          return {
            ...item,
            amount: item.amount + (0 * parseInt(next?.qty)),
            numbers_document: item.numbers_document + 1,
            percentage: ((item.numbers_document + 1)/data.length)*100        
          }
        }
        if (item.name.toLowerCase() === next?.status[0]?.status_type?.toLowerCase())
          return {
            ...item,
            amount: item.amount + (parseFloat(next?.final_price) * parseInt(next?.qty)),
            numbers_document: item.numbers_document + 1,
            percentage: ((item.numbers_document + 1)/data.length)*100
          };
        else return item;
  ...
  let total = data?.reduce(
    function (initial, next) {
      return {
        ...
        amount: initial.amount + (parseFloat(next?.final_price) * parseInt(next?.qty)),
      };
    },
          ...
          <StyledComponent>
            ...
            <StyledComponentInformation>
              ...
              <StyledTypography variant="h6">
                ...
                <StyledSpanTypography>costing</StyledSpanTypography>
              </StyledTypography>
              <StyledTypography variant="h6" style={{ color: 'rgb(0, 184, 217)' }}>
                {`Rp. ${fShortenNumber(total.amount)}`}
              </StyledTypography>
            </StyledComponentInformation>
          </StyledComponent>
