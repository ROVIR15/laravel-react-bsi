...
export default function UnstyledSelectsMultiple({ option, label, name, handleChange, ...rest}) {
  return (
    <div>
      <InputLabel sx={{ fontSize: '0.75rem'}}>{label}</InputLabel>
      <CustomMultiSelect name={name} defaultValue={['Size']} onChange={(value) => handleChange(name, value)}>
        {option.map(function(x){
          return (
            <CustomOptionGroup label={x.label}>
              {x.sub.map(function(y){
                return (
                  <StyledOption value={y.value}>
                    {y.value}
                  </StyledOption>
                )
              })}
