...
export default function SummaryPaper({ data }) {
  ...
  useEffect(() => {
    let result = data?.reduce(function (initial, next) {
      //check
      let targeted = initial?.map((item) => {
        if (item.name.toLowerCase() === next?.status?.status?.toLowerCase())
          return {
            ...item,
            amount: item.amount + parseFloat(next?.additionalCost),
  ...
  let total = data?.reduce(
    function (initial, next) {
      return {
        ...
        amount: initial.amount + parseFloat(next?.additionalCost)
      };
    },
          ...
          <StyledComponent>
            ...
            <StyledComponentInformation>
              ...
              <StyledTypography variant="h6">
                ...
                <StyledSpanTypography>invoice</StyledSpanTypography>
              </StyledTypography>
