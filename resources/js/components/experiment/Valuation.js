...
import { ListHead, ListToolbar, MoreMenu } from '../Table';
          ...
          let _obj = _arrangedData.reduce(
            function (initial, next) {
              return {
                ...
                m_total_produced: initial.m_total_produced + (parseInt(next?.qty) * parseFloat(next?.unit_price)),
                m_total_available: initial.m_total_available + ((parseInt(next?.qty) - parseInt(next?.shipped)) * parseFloat(next?.unit_price)),
                m_total_shipped: initial.m_total_shipped + (parseInt(next?.shipped) * parseFloat(next?.unit_price))
              };
            },
            {
              total_produced: 0,
  ...
  return (
    <Stack direction="column" spacing={2} m={'2ch'}>
      ...
      <Grid container direction="row" justifyContent="space-evenly" spacing={1}>
        <Grid item xs={3}>
          <PaperBox 
            title="Total Ready-Made Garments" 
            value={obj?.total_produced} 
          />
        </Grid>

        <Grid item xs={3}>
          <PaperBox 
            title="Shipped Garments"
            value={obj?.total_shipped}
          />
        </Grid>
