...
function processData(array) {
  if(isArray(array) && !isEmpty(array)){
    ...
    return array.reduce(function(initial, next){
      ...
      const {planned_output, realisation} = next?.plans.reduce((initial2, next2) => ({...initial2, planned_output: initial2?.planned_output + Math.round(next2?.total_expected_output), realisation: initial2.realisation + parseFloat(next2?.find_realisation_of_sewing?.total_output || 0)}), {planned_output: 0, realisation: 0})
      //detail of order
      const _breakdown = next?.plans?.map((item, index) => ({
        id: index,
        name: item?.find_realisation_of_sewing?.sales_order?.po_number,
        start_date: moment(item?.find_realisation_of_sewing?.date).format('ll'),
                  ...
                  {row?.history?.map((historyRow) => (
                    <TableRow key={historyRow.date}>
                      ...
                      <TableCell align="right">
                        {Math.round(historyRow.realisation)}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
