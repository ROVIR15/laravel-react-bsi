...
function DisplayBuyer({ placeHolder }) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = BUYERLIST.map((n) => n.name);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - BUYERLIST.length) : 0;

  const filteredData = applySortFilter(BUYERLIST, getComparator(order, orderBy), filterName);
          ...
          <Table size="small">
            ...
            <TableBody>
              {filteredData
                ...
                .map((row) => {
                  const { id, name, email, phone_number, address, city, province, country, postal_code } = row;
